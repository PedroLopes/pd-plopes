...
for ARG in $@
	do
		ACNT=$((ACNT + 1))
		case $ARG in "-d")DEBUG=1;;"-m")MIN_INPUT=$(echo $@ | cut -d " " -f $ACNT);;"-M")MAX_INPUT=$(echo $@ | cut -d " " -f $ACNT);;"-p")SUPER=1;;"-h")fhelp;;"--help")fhelp;;"-f")FORMAT=$(echo $@ | cut -d " " -f $ACNT);;"-t")TOR="torify";LOCATION="Tor Node ";;"-l")LENGTH=$(echo $@ | cut -d " " -f $ACNT);;"-n")PASSES=$(echo $@ | cut -d " " -f $ACNT);;"-q")QUIET="on";;"-o")OUTFILE=$(echo $@ | cut -d " " -f $ACNT);esac
	done
